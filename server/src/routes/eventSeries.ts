...
export function mountEventSeriesRoutes(api: Hono) {
  // -------------------- Event Series Routes --------------------
  const seriesRoutes = new Hono();
  seriesRoutes.use('*', authMiddleware);
  ...
  seriesRoutes.get('/', async (c) => {
    try {
      ...
      console.error('List event series error:', error);
      return c.json({ error: 'Failed to list event series' }, 500);
    }
  });
  ...
  seriesRoutes.post('/', async (c) => {
    try {
      ...
      console.error('Create event series error:', error);
      return c.json({ error: 'Failed to create event series' }, 500);
    }
  });

  // GET /event-series/:seriesId
  seriesRoutes.get('/:seriesId', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      ...
      return c.json({ error: 'Failed to get event series' }, 500);
    }
  });

  // PATCH /event-series/:seriesId
  seriesRoutes.patch('/:seriesId', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      const body = await c.req.json();
      ...
      console.error('Patch event series error:', error);
      return c.json({ error: 'Failed to update event series' }, 500);
    }
  });

  // DELETE /event-series/:seriesId
  seriesRoutes.delete('/:seriesId', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      ...
      console.error('Delete event series error:', error);
      return c.json({ error: 'Failed to delete event series' }, 500);
    }
  });

  // Areas nested under series
  const seriesAreasRoutes = new Hono();
  seriesAreasRoutes.use('*', authMiddleware);

  seriesAreasRoutes.get('/:seriesId/areas', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      const rows = await db
      ...
      console.error('List series areas error:', error);
      return c.json({ error: 'Failed to list series areas' }, 500);
    }
  });

  seriesAreasRoutes.put('/:seriesId/areas', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      const body = await c.req.json();
      ...
      console.error('Replace series areas error:', error);
      return c.json({ error: 'Failed to replace series areas' }, 500);
    }
  });

  seriesAreasRoutes.post('/:seriesId/areas', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      const body = await c.req.json();
      ...
      console.error('Add series area error:', error);
      return c.json({ error: 'Failed to add series area' }, 500);
    }
  });

  seriesAreasRoutes.delete('/:seriesId/areas/:areaId', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      ...
      console.error('Remove series area error:', error);
      return c.json({ error: 'Failed to remove series area' }, 500);
    }
  });

  // Preview occurrences and would-be events
  seriesRoutes.post('/:seriesId/preview', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      const body = await c.req.json();
      ...
      return c.json({ error: 'Failed to preview series' }, 500);
    }
  });

  // Generate events
  seriesRoutes.post('/:seriesId/generate', async (c) => {
    try {
      const seriesId = c.req.param('seriesId');
      const body = await c.req.json();
      const untilDate = String(body.untilDate || '').trim();
      ...
      return c.json({ error: 'Failed to generate series' }, 500);
    }
  });

  // Rule CRUD
  seriesRoutes.get('/:seriesId/rule', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      ...
      console.error('Get series rule error:', error);
      return c.json({ error: 'Failed to get series rule' }, 500);
    }
  });

  seriesRoutes.patch('/:seriesId/rule', async (c) => {
    try {
      const db = await getDatabase(getDatabaseUrl() || process.env.DATABASE_URL || 'postgresql://postgres:password@localhost:5502/postgres');
      const seriesId = c.req.param('seriesId');
      const body = await c.req.json();
      ...
      if ('frequency' in body) {
        const freq = String(body.frequency || '').trim().toUpperCase();
        if (!isValidFrequency(freq)) return c.json({ error: 'invalid frequency' }, 400);
      ...
      if ('byWeekdayMask' in body) {
        const mask = Number(body.byWeekdayMask);
        if (!isValidWeekdayMask(mask)) return c.json({ error: 'invalid byWeekdayMask' }, 400);
        patch.byWeekdayMask = mask;
      }
      patch.updatedAt = new Date();
      ...
      if (existing.length === 0) {
        const g: any = globalThis as any; let id: string | undefined = g?.crypto?.randomUUID?.();
        if (!id) { try { const nc = await import('node:crypto'); if (nc.randomUUID) id = nc.randomUUID(); } catch {} }
        if (!id) id = `srl_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
        const frequency = patch.frequency || 'WEEKLY';
        if (!isValidFrequency(frequency)) return c.json({ error: 'invalid frequency' }, 400);
        const interval = patch.interval ?? 1;
        if (!Number.isInteger(interval) || interval < 1) return c.json({ error: 'interval must be >= 1' }, 400);
        const byWeekdayMask = patch.byWeekdayMask ?? 0;
        if (!isValidWeekdayMask(byWeekdayMask)) return c.json({ error: 'invalid byWeekdayMask' }, 400);
        const inserted = await db.insert(schema.eventSeriesRules).values({ id, seriesId, frequency, interval, byWeekdayMask }).returning();
        return c.json(inserted[0], 201);
      } else {
        const updated = await db.update(schema.eventSeriesRules).set(patch).where(eq(schema.eventSeriesRules.id, existing[0].id)).returning();
        return c.json(updated[0]);
      }
    } catch (error) {
      console.error('Patch series rule error:', error);
      return c.json({ error: 'Failed to update series rule' }, 500);
    }
  });

  // Mount series routes and nested areas
  api.route('/event-series', seriesRoutes);
  api.route('/event-series', seriesAreasRoutes);
}
